...
const _sfc_main = /* @__PURE__ */ defineComponent({
  ...
  async setup(__props) {
    ...
    const { data: list, refresh, status } = ([__temp, __restore] = withAsyncContext(() => useFetch("/api/admin/companies", "$1XiBtTqpPz")), __temp = await __temp, __restore(), __temp);
    const deletingId = ref(null);
    return (_ctx, _push, _parent, _attrs) => {
      const _component_NuxtLink = __nuxt_component_0$1;
      _push(`<div${ssrRenderAttrs(mergeProps({ class: "max-w-6xl mx-auto" }, _attrs))}><div class="flex items-center justify-between mb-8"><h1 class="text-3xl font-black text-gray-900 tracking-tight"> Companies </h1>`);
      ...
      if (unref(status) === "pending") {
        ...
        _push(`<div class="bg-white border border-gray-200 rounded-xl overflow-hidden shadow-sm"><table class="w-full text-left border-collapse"><thead class="bg-gray-50 border-b border-gray-200"><tr><th class="px-6 py-4 text-xs font-bold uppercase text-gray-500"> Company </th><th class="px-6 py-4 text-xs font-bold uppercase text-gray-500"> Industry </th><th class="px-6 py-4 text-xs font-bold uppercase text-gray-500"> Stage </th><th class="px-6 py-4 text-xs font-bold uppercase text-gray-500"> Status </th><th class="px-6 py-4 text-xs font-bold uppercase text-gray-500 text-right"> Actions </th></tr></thead><tbody class="divide-y divide-gray-100"><!--[-->`);
        ssrRenderList(unref(list), (item) => {
          _push(`<tr class="group hover:bg-gray-50/50 transition-colors"><td class="px-6 py-4"><div class="flex items-center gap-3"><div class="w-8 h-8 rounded bg-gray-100 flex items-center justify-center font-bold text-xs border border-gray-200">`);
          if (item.logo && item.logo.startsWith("http")) {
            _push(`<img${ssrRenderAttr("src", item.logo)} class="w-full h-full object-cover rounded">`);
          } else {
            _push(`<span>${ssrInterpolate(item.name.charAt(0))}</span>`);
          }
          _push(`</div><div><p class="text-sm font-bold text-gray-900">${ssrInterpolate(item.name)}</p><a${ssrRenderAttr("href", item.website)} target="_blank" class="text-xs text-gray-400 hover:text-blue-600">${ssrInterpolate(item.website)}</a></div></div></td><td class="px-6 py-4"><span class="text-xs font-medium text-gray-600 bg-gray-100 px-2 py-1 rounded">${ssrInterpolate(item.industry)}</span></td><td class="px-6 py-4 text-sm text-gray-500">${ssrInterpolate(item.stage)}</td><td class="px-6 py-4"><span class="${ssrRenderClass([
            item.status === "approved" ? "bg-green-50 text-green-700 border-green-100" : "bg-orange-50 text-orange-700 border-orange-100",
            "text-[10px] font-bold uppercase px-2 py-1 rounded border"
          ])}">${ssrInterpolate(item.status)}</span></td><td class="px-6 py-4 text-right flex justify-center">`);
          _push(ssrRenderComponent(_component_NuxtLink, {
        ...
        if (!unref(list) || unref(list).length === 0) {
          _push(`<div class="p-12 text-center"><p class="text-gray-400 text-sm"> No companies found. Add your first one! </p></div>`);
        } else {
          _push(`<!---->`);
        }
        _push(`</div>`);
...
//# sourceMappingURL=index-BucOOXXd.mjs.map
