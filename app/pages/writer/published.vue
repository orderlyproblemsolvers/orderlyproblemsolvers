<template>
  <div class="text-white">
    <div class="flex flex-wrap gap-4 justify-between items-center mb-8">
      <h1 class="font-heading text-3xl">Published Posts</h1>
      <NuxtLink 
        to="/writer/new"
    ...
    <div v-if="loading" class="text-center text-gray-400">Loading published posts...</div>
    
    <div v-else-if="errorMsg" class="text-red-400 bg-red-900/50 border border-red-800 rounded-lg p-4">
      {{ errorMsg }}
    </div>

    <div v-else-if="publishedPosts.length === 0" class="text-center text-gray-400 border-2 border-dashed border-gray-700 rounded-lg p-12">
      <p class="font-heading text-xl mb-2">No published posts found.</p>
      <p class="font-ibm-plex-mono">You haven't published any posts yet.</p>
    </div>
        ...
        <table class="w-full min-w-full divide-y divide-gray-800">
          <thead class="bg-slate-800">
            <tr>
              ...
              <th class="px-6 py-3 text-left text-xs font-ibm-plex-mono font-bold text-gray-400 uppercase tracking-wider">Published Date</th>
          ...
          <tbody class="divide-y divide-gray-800">
            <tr v-for="post in publishedPosts" :key="post.id" class="hover:bg-slate-800/50">
      ...
      <div class="md:hidden space-y-4">
        ...
        >
          ...
          <div class="flex flex-wrap items-center gap-x-4 gap-y-2 text-sm mt-3">
            ...
            <span class="text-gray-400 ml-auto">Published: {{ new Date(post.created_at).toLocaleDateString() }}</span>
          </div>
        </div>
      </div>
...
const publishedPosts = ref([]) // Renamed from drafts
const loading = ref(true)
const errorMsg = ref(null)

const fetchPublishedPosts = async (userId) => {
  try {
    loading.value = true
    errorMsg.value = null
    ...
    const { data, error } = await supabase
      ...
      .eq('status', 'published') // <-- The key change is here
      .order('created_at', { ascending: false })
      
    if (error) throw error
    publishedPosts.value = data // Renamed
...
watch(user, (currentUser) => {
  if (currentUser && currentUser.id) {
    fetchPublishedPosts(currentUser.id)
  }
}, { immediate: true })
</script>