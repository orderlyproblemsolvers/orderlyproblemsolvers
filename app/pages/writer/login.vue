    ...
    <div class="relative z-10 w-full max-w-md">
      <div class="text-center mb-8">
        ...
        <h1 class="text-3xl md:text-4xl font-heading font-bold text-white mb-2">
          Writer <span class="text-transparent bg-clip-text bg-gradient-to-r from-yellow-400 to-yellow-300">Portal</span>
        </h1>
...
<script setup>
definePageMeta({
  layout: 'login', // Correctly uses a separate layout
})
...
const handleLogin = async () => {
  try {
    ...
    // Check if authenticated user is in the writers table
    ...
    if (writerError || !writerData) {
      // Sign out the user since they're not an active writer
      await supabase.auth.signOut()
      throw new Error('Access denied. You do not have writer privileges.')
    }

    // Success - writer is verified
    successMessage.value = 'Login successful! Redirecting...'
    
    if (rememberMe.value) {
      localStorage.setItem('writer_email', email.value)
    }

    // Redirect to admin dashboard after short delay
    setTimeout(() => {
      router.push('/writer') // This redirect is correct
    }, 1000)
...
const loadRememberedEmail = () => {
  ...
  const rememberedEmail = localStorage.getItem('writer_email')
  if (rememberedEmail) {
...
onMounted(() => {
  // We removed checkExistingSession() as the global middleware handles it.
  loadRememberedEmail()
})
</script>
