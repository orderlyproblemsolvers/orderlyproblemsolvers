...
const addTech = () => {
  ...
  const exists = form.value.stack.find(
      ...
      <div class="grid grid-cols-2 gap-6">
        <div>
          ...
          >
            ...
            <option>Media & Creator Tech</option>
            <option>HR & Future of Work</option>
            <option>Sales & Marketing Tech</option>
            <option>Payments & Infrastructure</option>
            <option>Blockchain & Web3</option>
            <option>Smart Cities & IoT</option>
            <option>Enterprise Software</option>
            <option>Social Impact & Non-Profit Tech</option>
            <option>DefenseTech</option>
            <option>Insurance Tech</option>
      ...
      <div class="bg-gray-50 p-6 rounded-xl border border-gray-200">
        <div class="flex items-center justify-between mb-4">
          <label class="block text-xs font-bold uppercase text-gray-500"
            >Tech Stack & Solutions [Image of tech stack diagram]
          </label>
          <span
            class="text-[10px] text-gray-400 bg-white border border-gray-200 px-2 py-0.5 rounded-full"
            >{{ form.stack.length }} added</span
          >
        </div>
        ...
        <div class="flex flex-col md:flex-row gap-3 mb-4">
          <div class="md:w-1/2">
            <select
              ...
              class="w-full px-3 py-2.5 text-sm border border-gray-300 rounded-lg focus:border-black outline-none bg-white"
            >
              <option
                v-for="cat in SOLUTION_CATEGORIES"
                :key="cat"
                :value="cat"
              >
                {{ cat }}
        ...
        <div v-if="form.stack.length > 0" class="flex flex-wrap gap-2">
          ...
          >
            <div class="flex flex-col leading-none pr-1">
              <span
                class="text-[9px] text-gray-400 uppercase font-bold mb-0.5"
                >{{ item.category }}</span
              >
        ...
        <div v-if="form.videos.length > 0" class="space-y-2">
          ...
          >
            ...
            <button
              @click="removeVideo(i)"
              type="button"
              class="text-red-500 hover:text-red-700 font-bold text-xs transition-colors"
            >
              Remove
            </button>
          </div>
